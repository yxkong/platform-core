...
public class CTYunFileUploadService extends AbstractUploadFileService  {
     ...
     */
    @Override
    public void upload(String module, String bizNo, String path, final String uploadFileName, InputStream is) {
        try {
            PutObjectRequest request = new PutObjectRequest(getCtyun().getBucketName(),getObjectName(path,uploadFileName), is, new ObjectMetadata());
    ...
    public String getUrl(SysUploadFileDto dto) {
        ...
        GeneratePresignedUrlRequest shareUrlRequest = new GeneratePresignedUrlRequest(bucketName, dto.getObjectName());
