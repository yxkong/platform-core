...
public class DataScopeAspect {
     ...
     * @param params
     */
    private static void baseDO(DataScope dataScope, Object params) {
        if (!(params instanceof BaseDO)){
            return;
        }
        BaseDO baseEntity = (BaseDO) params;
        // 获取当前的用户
        LoginUserInfo loginUser = LoginUserInfoUtil.getLoginUserInfo();
        if (AuthUtil.isSuperAdmin()) {
        ...
        String scopeSql = " AND (" + sqlString.substring(4) + ")";
        baseEntity.getParams().put(DATA_SCOPE, scopeSql);
        log.info("数据权限生成sql:{}", scopeSql);
    }
